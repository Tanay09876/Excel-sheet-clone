...
document.getElementById("pasteBtn").addEventListener("click", async () => {
  try {
    ...
    if (currentCell) {
      ...
      const cellId = currentCell.dataset.id;
      cellData[cellId] = cellData[cellId] || {};
      cellData[cellId].value = text;
    } else {
      alert("Click a cell before pasting.");
    }
  } catch (err) {
...
//     spreadsheet.appendChild(createSheetHeader('div'));
//    for(let col=0; col<20; col++){
//     const colHeader = document.createElement('div');
//     colHeader.className = 'header';
//     colHeader.textContent = String.fromCharCode(65 + col);
...
//         rowHeader.className = 'row-header';
//         rowHeader.textContent = row;
//         rowHeader.appendChild(rowHeader);
...
//         }
//     }
//  containerId.appendChild(spreadsheet);
...
//         let rowData= [];
//         for(let col=0; col<20; col++){
//             let cellId = `S_${String.fromCharCode(65 + col)}${row}`;
